$(function() {
  ...
  // Get customer parameter from URL query string
  ...
  // Define gradients for chart lines
  ...
  $.ajax({
    type: "POST",
    url: "../../vertical/ajaxComponents/alertdashboard1.php",
    data: { customer: customer },
    success: function (response) {
        var d = JSON.parse(response);
        if (d.data_a) {
            var categories = JSON.parse(d.categories);
            var data_a = d.data_a.split(',').map(Number);
            var data_b = d.data_b.split(',').map(Number);

            // Chart.js initialization with fetched data
            var myChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    tooltips: {
                        ...
                        mode: 'index',
                        intersect: false,
                        itemRadius: 8, // Increase the radius of points for tooltips
                        itemHoverRadius: 12, // Increase the hover radius of points
                        callbacks: {
